...
class Action extends DbMysql
{
     ...
     * 获取分类树
     +----------------------------------------------------------
     */
    function get_category_tree($pid = 0){

        $sql = "SELECT * FROM " . $this->table('article_category') . " WHERE pid = ".$pid." ORDER BY sort ASC,cat_id ASC";
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query))
        {
            $item = array (
            ...
            $get_sub = $this->get_one("SELECT cat_id FROM " . $this->table('article_category') . " WHERE pid = '" . $row['cat_id'] . "'");
            if ($get_sub) {
                $item['sub'] = $this->get_category_tree($row['cat_id']);
            }
